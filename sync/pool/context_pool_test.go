...
func ExampleContextPool_WithCancelOnError() {
	...
	fmt.Println(err)
	// Output:
	// [[ExampleContextPool_WithCancelOnError.func1] (context_pool_test.go#25) I will cancel all other tasks!]
}
				...
				for i := 0; i < 100; i++ {
					p.Go(func(context.Context) error {
						...
						if cur > int64(maxConcurrent) {
							return errors.New(fmt.Sprintf("expected no more than %d concurrent goroutine\n", maxConcurrent))
